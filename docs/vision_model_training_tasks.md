# 大模型视觉训练任务 - 航班预订

> 💡 **验证工具**：
> - 命令行：[CLI_VALIDATION_GUIDE.md](CLI_VALIDATION_GUIDE.md)
> - REST API：[API_GUIDE.md](API_GUIDE.md)

本文档定义了用于训练大模型视觉理解和操作能力的航班预订任务。大模型需要像人类一样，通过观察屏幕、理解界面元素、点击按钮来完成预订任务。

---

## 任务目标

训练大模型具备以下能力：
1. **视觉理解**：识别网页上的表单、按钮、弹窗等UI元素
2. **任务规划**：理解用户需求，规划完成任务的步骤
3. **交互操作**：模拟鼠标点击、键盘输入等操作
4. **结果验证**：确认任务是否成功完成

---

## 核心任务定义

### 任务：预订航班机票

**用户指令示例**：
```
帮我订一张从深圳到武汉的机票，1月15日出发
```

**任务要求**：
大模型需要：
1. 理解用户意图（预订机票、出发城市、到达城市、日期）
2. 在网页上找到搜索框并输入信息
3. 点击搜索按钮查找航班
4. 从搜索结果中选择合适的航班
5. 填写乘客信息表单
6. 处理保险推荐弹窗
7. 处理会员注册弹窗
8. 完成支付流程
9. 确认预订成功

**成功标准**：
```ruby
# 数据库中新增一条预订记录
booking = Booking.where(user_id: user_id).order(created_at: :desc).first

# 验证航班路线
booking.flight.departure_city == "深圳" &&
booking.flight.arrival_city == "武汉" &&
booking.flight.departure_time.to_date == Date.parse("2025-01-15")

# 验证预订状态
booking.status == "paid"  # 已完成支付
```

---

## 任务变体

### 变体 1：基础预订（指定路线和日期）

**用户指令**：
```
帮我订1月15号从北京到上海的机票
```

**任务参数**：
```ruby
{
  departure_city: "北京",
  arrival_city: "上海",
  departure_date: "2025-01-15",  # 必填
  should_complete_payment: true
}
```

**成功评估代码**：
```ruby
validator = FlightBookingTaskValidator.new(task_params)
validator.record_initial_state  # 记录初始状态

# 大模型执行操作...

result = validator.result
# result[:valid] => true/false
# result[:errors] => ["错误信息1", "错误信息2", ...]
```

---

### 变体 2：指定乘客信息

**用户指令**：
```
帮我订1月20号从上海到北京的机票，乘客姓名张三，手机号13800138000
```

**任务参数**：
```ruby
{
  departure_city: "上海",
  arrival_city: "北京",
  departure_date: "2025-01-20",
  passenger_name: "张三",
  contact_phone: "13800138000",
  should_complete_payment: true
}
```

---

### 变体 3：指定保险需求

**用户指令**：
```
帮我订1月25号从深圳到武汉的机票，要买保险
```

**任务参数**：
```ruby
{
  departure_city: "深圳",
  arrival_city: "武汉",
  departure_date: "2025-01-25",
  insurance_required: true,
  should_complete_payment: true
}
```

---

### 变体 4：只填表单，不支付

**用户指令**：
```
帮我填写1月30号从杭州到成都的机票预订表单，不用支付
```

**任务参数**：
```ruby
{
  departure_city: "杭州",
  arrival_city: "成都",
  departure_date: "2025-01-30",
  should_complete_payment: false
}
```

**成功标准**：
```ruby
booking.status == "pending"  # 待支付状态即可
```

---

## 任务验证流程

### 1. 记录初始状态

```ruby
validator = FlightBookingTaskValidator.new(
  user_id: 1,
  departure_city: "深圳",
  arrival_city: "武汉",
  departure_date: "2025-01-15",
  should_complete_payment: true
)

# 记录任务开始前的预订数量
validator.record_initial_state
```

### 2. 大模型执行任务

大模型通过视觉识别和操作完成预订流程：
- 观察屏幕，识别搜索框
- 输入"深圳"、"武汉"、"2025-01-15"
- 点击搜索按钮
- 识别航班列表，选择航班
- 填写乘客信息表单
- 处理各种弹窗（保险、会员）
- 完成支付流程

### 3. 验证任务结果

```ruby
result = validator.result

if result[:valid]
  puts "✅ 任务完成：大模型成功预订了从深圳到武汉的机票"
else
  puts "❌ 任务失败："
  result[:errors].each { |error| puts "  - #{error}" }
end
```

---

## 验证规则说明

### 规则 1：新预订记录

验证是否创建了新的预订记录：
```ruby
# 当前预订数量 > 初始预订数量
Booking.where(user_id: user_id).count > initial_count
```

### 规则 2：航班路线匹配

验证预订的航班是否符合要求：
```ruby
booking.flight.departure_city == task_params[:departure_city] &&
booking.flight.arrival_city == task_params[:arrival_city]
```

### 规则 3：日期匹配（必填）

```ruby
# 日期是必填参数
task_params[:departure_date].present? &&
booking.flight.departure_time.to_date == Date.parse(task_params[:departure_date])
```

### 规则 4：乘客信息匹配（如果指定）

```ruby
if task_params[:passenger_name]
  booking.passenger_name == task_params[:passenger_name]
end

if task_params[:contact_phone]
  booking.contact_phone == task_params[:contact_phone]
end
```

### 规则 5：保险选择匹配（如果指定）

```ruby
if task_params[:insurance_required] == true
  booking.insurance_type != "无保障"  # 必须购买保险
elsif task_params[:insurance_required] == false
  booking.insurance_type == "无保障"  # 不能购买保险
end
```

### 规则 6：支付状态匹配

```ruby
if task_params[:should_complete_payment] == true
  booking.status == "paid"  # 必须完成支付
else
  booking.status == "pending"  # 可以不支付
end
```

---

## 使用示例

### 示例 1：完整预订流程

```ruby
# 定义任务
task = {
  user_id: 1,
  departure_city: "深圳",
  arrival_city: "武汉",
  departure_date: "2025-01-15",
  passenger_name: "张三",
  contact_phone: "13800138000",
  insurance_required: true,
  should_complete_payment: true
}

# 创建验证器并记录初始状态
validator = FlightBookingTaskValidator.new(task)
validator.record_initial_state

# === 大模型在此执行任务 ===
# 1. 视觉识别页面元素
# 2. 填写搜索表单
# 3. 选择航班
# 4. 填写乘客信息
# 5. 购买保险
# 6. 完成支付

# 验证结果
result = validator.result

puts "任务验证结果："
puts "通过：#{result[:valid]}"
if result[:errors].any?
  puts "错误："
  result[:errors].each { |error| puts "  - #{error}" }
end
```

### 示例 2：批量任务测试

```ruby
# 定义多个测试任务
tasks = [
  {
    name: "深圳到武汉",
    params: { departure_city: "深圳", arrival_city: "武汉" }
  },
  {
    name: "北京到上海（指定日期）",
    params: { departure_city: "北京", arrival_city: "上海", departure_date: "2025-01-20" }
  },
  {
    name: "广州到深圳（需要保险）",
    params: { departure_city: "广州", arrival_city: "深圳", insurance_required: true }
  }
]

# 依次执行并验证
results = []
tasks.each do |task|
  validator = FlightBookingTaskValidator.new(task[:params].merge(user_id: 1, should_complete_payment: true))
  validator.record_initial_state
  
  # 大模型执行...
  
  result = validator.result
  results << {
    task_name: task[:name],
    valid: result[:valid],
    errors: result[:errors]
  }
end

# 打印汇总
puts "\n任务完成统计："
puts "总任务数：#{results.count}"
puts "成功：#{results.count { |r| r[:valid] }}"
puts "失败：#{results.count { |r| !r[:valid] }}"
```

---

## 与 AndroidWorld 的对比

| 特性 | AndroidWorld | 本项目（航班预订） |
|------|-------------|-------------------|
| **目标** | 训练模型操作 Android 应用 | 训练模型操作 Web 应用 |
| **交互方式** | 触摸、滑动、输入文字 | 鼠标点击、键盘输入 |
| **任务粒度** | 单个操作（如创建播放列表） | 完整业务流程（预订机票） |
| **验证方式** | 检查应用内状态 | 检查数据库记录 |
| **噪音数据** | 无关文件、联系人等 | 其他用户的预订记录 |

---

## 噪音数据设计

为了测试模型的鲁棒性，应在数据库中添加"噪音"数据：

```ruby
# 添加其他路线的航班（不应被选择）
Flight.create!(
  departure_city: "北京",
  arrival_city: "上海",
  departure_time: "2025-01-15 10:00",
  price: 500
)

# 添加其他用户的预订（不应影响验证）
Booking.create!(
  user_id: 999,  # 不同的用户
  flight_id: some_flight.id,
  passenger_name: "李四",
  status: :paid
)

# 添加当前用户的历史预订（不应被计入新预订）
Booking.create!(
  user_id: 1,  # 当前用户
  flight_id: another_flight.id,
  created_at: 1.day.ago,  # 早于任务开始时间
  status: :completed
)
```

验证器只会检查**任务开始后新创建的**、**属于指定用户的**预订记录。

---

## 扩展方向

1. **多步骤任务**：先搜索、再预订、最后修改订单
2. **异常处理**：航班已满、支付失败等场景
3. **多用户场景**：同时为多个用户预订
4. **时间约束**：限定完成任务的时间
5. **复杂条件**：经济舱、中转、特定时间段等

---

## 总结

本验证框架的核心思想：

1. ✅ **任务导向**：给大模型一个明确的任务目标
2. ✅ **结果验证**：只检查最终数据库状态，不关心中间步骤
3. ✅ **简单明了**：一个任务 = 一个目标 = 一个验证规则
4. ✅ **可扩展**：轻松添加新的任务变体和验证规则

大模型需要自己理解UI、规划步骤、处理异常，最终只要数据库里有正确的预订记录，就算任务成功。
