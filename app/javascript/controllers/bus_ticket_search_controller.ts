...
export default class extends Controller {
  ...
  declare readonly timeButtonTarget: HTMLElement
  ...
  currentSortField: string = 'default'
  ...
  // 重置按钮样式
  resetButtonStyle(button: HTMLElement): void {
  ...
  // 高亮按钮
  highlightButton(button: HTMLElement, label: string): void {
    const icon = button.querySelector('.w-6.h-6')
    const text = button.querySelector('.text-\\[11px\\]')
