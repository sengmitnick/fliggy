...
# 验证用例97: 给张三兑换东京科摩思药妆店优惠券（满额立减7%）
#
# 任务描述:
#   用户想兑换东京科摩思药妆店的优惠券，要求选择折扣最高的券。
#   具体要求：品牌名包含"科摩思"，门店城市为"东京"，门店名包含"歌舞伎町"。
#   Agent 需要在符合条件的优惠券中，选择discount_value最高的优惠券进行兑换。
#
# 业务流程（4个关键步骤）：
#   1. 搜索品牌名包含"科摩思"的品牌
#   2. 筛选该品牌下门店城市为"东京"的门店
#   3. 筛选门店名包含"歌舞伎町"的门店优惠券
#   4. 在符合条件的优惠券中，选择discount_value最高的券进行兑换
#
# 复杂度分析（4个关键点）：
#   1. 需要理解品牌筛选：品牌名包含"科摩思"
#   2. 需要理解门店筛选：门店城市="东京"且门店名包含"歌舞伎町"
#   3. 需要理解优惠券选择：选择discount_value最高的券
#   4. 需要理解优惠券兑换操作：创建UserCoupon记录，状态为claimed
#   ❌ 不能随机选择：必须精确筛选并选择折扣值最高的优惠券
#
# 评分标准（5项，总计100分）：
#   - 优惠券已兑换（20分）
#   - 品牌正确（科摩思）（25分）
...
module V051V100
  class V097RedeemTokyoCosmosCosmeticsShopCouponValidator < BaseValidator
    ...
    self.description = '兑换东京科摩思药妆店优惠券（满额立减7%）'
