...
# 验证用例96: 给张三预订武汉经典一日包车游（经济7座，8小时，家庭出行，3天后出发）
#
# 任务描述:
#   用户想预订武汉经典一日包车游，家庭5人出行，3天后出发。
#   要求选择经济7座车型（座位数≥5人），包车时长8小时（一日游标准服务时长）。
#   Agent 需要在符合条件的路线和车型中，选择合适的包车产品，并填写联系人信息完成预订。
#
# 业务流程（5个关键步骤）：
#   1. 搜索武汉城市的包车路线
#   2. 筛选路线名包含"经典一日游"的路线
#   3. 选择经济7座车型（座位数≥5人，适合家庭出行）
#   4. 设置服务时长为8小时（一日游标准时长）
#   5. 填写出发日期（3天后）、联系人信息并提交订单
#
# 复杂度分析（5个关键点）：
#   1. 需要理解路线筛选：路线名包含"经典一日游"
#   2. 需要理解车型选择：经济7座，座位数≥5人（能容纳家庭出行）
#   3. 需要理解服务时长：8小时（一日游标准服务时长）
#   4. 需要理解出发日期计算：3天后（Date.current + 3.days）
...
#   - 车型正确（经济7座，座位数≥5）（20分）
#   - 服务时长正确（8小时）（15分）
#   - 订单信息完整（联系人、电话、出发日期3天后、出发时间格式、预订模式）（15分）
#   - 价格计算正确（10分）
module V051V100
  class V096BookWuhanClassicOneDayCharteredTourValidator < BaseValidator
    ...
    self.title = '给张三预订武汉经典一日包车游（经济7座，8小时，家庭出行，3天后出发）'
    self.description = '预订武汉经典一日包车游（经济7座，家庭出行，3天后出发）'
    ...
    def prepare
    ...
    
      {
        ...
        hint: "1. 在包车游搜索页选择武汉城市\n2. 浏览并选择包含'经典一日游'的路线\n3. 在车型选择页选择8小时服务时长\n4. 选择'经济7座'车型（适合5人家庭出行）\n5. 填写联系人信息并提交订单",
    ...
    def verify
      ...
      # 断言2: 路线正确（权重15%）
      add_assertion "路线正确（武汉经典一日游）", weight: 15 do
      ...
      # 断言5: 订单信息完整（权重15%）
      # 验证联系人信息、出发日期（3天后）、出发时间格式、乘客数量、预订模式
