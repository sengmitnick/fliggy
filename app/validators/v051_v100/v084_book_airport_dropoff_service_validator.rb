...
# 验证用例84: 给张三预订上海送机服务（人民广场→浦东T1，舒适5座，明天早上6点）
# 
# 任务描述:
...
#   5. 用车时间：明天早上6点（pickup_datetime）
#   6. 浏览舒适5座车型套餐，选择价格最低的
# 
# 复杂度分析:
...
# 评分标准:
#   - 创建了送机订单 (15分)
#   - 服务类型正确（airport_dropoff + to_airport）(10分)
#   - 上车点和下车点正确（人民广场→浦东T1）(10分)
#   - 车辆类型正确（comfort_5 舒适5座）(10分)
#   - 联系人信息正确（张三）(5分)
#   - 选择了该车型中价格最低的套餐 (30分)
#   - 订单价格正确 (5分)
#   - 用车时间正确（明天早上6点）(15分)
module V051V100
  class V084BookAirportDropoffServiceValidator < BaseValidator
    ...
    self.title = '给张三预订上海送机服务（人民广场→浦东T1，舒适5座，明天早上6点）'
    self.description = '预订上海送机服务（人民广场→浦东T1航站楼，舒适5座，明天早上6点）'
    self.timeout_seconds = 240
  
    def prepare
      @service_type = 'to_airport'  # 送机服务
      @transfer_type = 'airport_dropoff'  # 送机 = airport_dropoff
      @departure_city = '上海'  # 航班出发城市
      @arrival_city = '北京'  # 航班降落城市
      @departure_airport = '浦东国际机场T1航站楼'  # 起飞机场（下车点，通过航班搜索确定）
      @pickup_location = '人民广场接送服务站'  # 上车点（市区地址，上海）
    ...
    def verify
      add_assertion "创建了送机订单", weight: 15 do
        @transfer = Transfer
      ...
      add_assertion "服务类型正确（airport_dropoff + to_airport）", weight: 10 do
      ...
      add_assertion "上车点和下车点正确（人民广场→浦东T1）", weight: 10 do
        expect(@transfer.location_from).to include(@pickup_location),
          "上车点错误。期望包含: #{@pickup_location}（市区地址）, 实际: #{@transfer.location_from}"
        
        # 验证下车点包含浦东和T1
        expect(@transfer.location_to).to include('浦东'),
          "下车点错误（缺少浦东）。期望包含: 浦东国际机场T1航站楼, 实际: #{@transfer.location_to}"
        expect(@transfer.location_to).to include('T1'),
          "下车点错误（缺少T1）。期望: T1航站楼，实际: #{@transfer.location_to}。注意：上海浦东有T1和T2两个航站楼，需要确认是T1"
      end
    
      add_assertion "车辆类型正确（comfort_5 舒适5座）", weight: 10 do
      ...
      add_assertion "用车时间正确（明天早上6点）", weight: 15 do
        ...
        # 验证时间是6点（允许一些误差，比如5:00-7:00之间）
        actual_hour = actual_datetime.hour
        expect(actual_hour).to be_between(5, 7).inclusive,
          "用车时间错误。期望: 明天早上6:00左右，实际: #{actual_datetime.strftime('%Y-%m-%d %H:%M')}"
      end
    end
