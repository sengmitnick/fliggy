...
# 验证用例99: 给张三囤货广州地区酒店套餐（1晚，1份，含早套餐）
# 
# 任务描述:
#   用户想囤货购买广州地区的酒店套餐，购买1份1晚套餐，先囤货后预约。
#   要求从套餐的多个选项中选择含早套餐选项（包含双人早餐），而非标准套餐。
#   Agent 需要搜索符合条件的套餐，对比不同选项，识别并选择含早套餐完成囤货购买。
# 
# 业务流程（5个关键步骤）：
#   1. 搜索广州地区的酒店套餐产品
#   2. 筛选1晚的套餐（night_count=1）
#   3. 查看该套餐的多个选项（标准套餐、含早套餐等）
#   4. 选择含早套餐选项（包含双人早餐）
#   5. 使用囤货模式（stockup）购买1份，填写联系人信息并提交订单
# 
# 复杂度分析（5个关键点）：
#   1. 需要理解城市筛选：广州地区的酒店套餐
#   2. 需要理解套餐晚数：1晚（night_count=1）
#   3. 需要理解预约模式：stockup（囤货，先购买后预约，不需指定入住日期）vs instant（立即预约，需指定日期）
#   4. 需要理解套餐选项差异：对比标准套餐和含早套餐的服务内容
#   5. 需要理解含早套餐特征：选项名称包含"含早"，包含双人早餐服务
#   ❌ 不能随机选择：必须精确识别并选择含早套餐选项
# 
# 评分标准（6项，总计100分）：
#   - 订单已创建（20分）
#   - 城市正确（广州）（15分）
...
module V051V100
  class V099BookGuangzhouHotelPackageLuxuryOptionValidator < BaseValidator
    ...
    self.title = '给张三囤货广州地区酒店套餐（1晚，1份，含早套餐）'
    self.description = '囤货广州地区酒店套餐（1晚，1份，含早套餐）'
