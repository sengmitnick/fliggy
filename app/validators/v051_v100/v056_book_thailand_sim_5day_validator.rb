...
# 验证用例: 王芳要去泰国7天，帮她买一张7天有效期、无限量流量的SIM卡，邮寄到广州天河
# 
# 任务描述:
...
module V051V100
  class V056BookThailandSim5dayValidator < BaseValidator
    ...
    self.title = '王芳要去泰国7天，帮她买一张7天有效期、无限量流量的SIM卡，邮寄到广州天河'
    self.description = '王芳要去泰国7天，帮她买一张7天有效期、无限量流量的SIM卡，邮寄到广州天河'
    ...
    def prepare
      ...
      # 王芳使用王芳的广州天河地址
      @recipient_address = user.addresses.find_by!(name: '王芳', data_version: 0)
      ...
      # 返回给 Agent 的任务信息
      {
        task: "请购买一张泰国7天无限量流量的SIM卡（数量1张，邮寄到广州天河）",
        region: @region,
        validity_days: @validity_days,
    ...
    def verify
      ...
      # 断言7: 收货地址正确（邮寄到广州天河）
      add_assertion "收货地址正确（邮寄到广州天河）", weight: 25 do
    ...
    # 模拟 AI Agent 操作：购买泰国7天无限量流量SIM卡
    def simulate
      # 1. 查找测试用户（数据包中已创建）
      user = User.find_by!(email: 'demo@travel01.com', data_version: 0)
