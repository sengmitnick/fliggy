...
# 验证用例100: 给张三立即预约北京地区酒店套餐（2晚，含早餐，下周一入住）
# 
# 任务描述:
#   用户想立即预约北京地区的酒店套餐，入住时间为下周一开始，连住2晚。
#   要求选择包含早餐的套餐选项（含早套餐或豪华套餐），使用立即预约模式（非囤货模式）。
#   Agent 需要搜索符合条件的套餐，计算下周一的日期，选择合适的酒店，并指定具体的入住日期完成预订。
# 
# 业务流程（6个关键步骤）：
#   1. 搜索北京地区的酒店套餐产品
...
#   5. 计算并设置入住日期为下周一（check_in_date），连住2晚（check_out_date=下周一+2天）
#   6. 填写联系人信息并提交订单
# 
# 复杂度分析（6个关键点）：
#   1. 需要理解城市筛选：北京地区的酒店套餐
#   2. 需要理解套餐晚数：2晚（night_count=2）
#   3. 需要理解预约模式：instant（立即预约，需指定日期和酒店）vs stockup（囤货，不指定日期）
#   4. 需要理解套餐选项：从多个选项中选择含早餐的（含早套餐 > 豪华套餐 > 标准套餐）
#   5. 需要理解日期计算：下周一（Date.current.next_occurring(:monday)），check_out_date=下周一+2天
#   6. 需要理解联系人信息填写：使用乘客信息中的张三
#   ❌ 不能随机选择：必须精确选择含早餐选项、正确计算下周一的日期
# 
# 评分标准（8项，总计100分）：
#   - 订单已创建（20分）
#   - 城市正确（北京）（10分）
#   - 套餐晚数正确（2晚）（10分）
#   - 预约模式正确（instant立即预约）（10分）
#   - 联系人信息正确（张三）（10分）
#   - 选择了含早餐的套餐选项（15分）
#   - 入住日期正确（下周一开始，连住2晚）（15分）
#   - 订单价格和数量正确（10分）
# 
# 使用方法:
#   # 准备阶段
#   POST /api/tasks/v100_instant_book_beijing_hotel_next_week_validator/start
#   
#   # Agent 通过界面操作完成立即预约...
#   
#   # 验证结果
...
module V051V100
  class V100InstantBookBeijingHotelNextWeekValidator < BaseValidator
    ...
    self.title = '给张三立即预约北京地区酒店套餐（2晚，含早餐，下周一入住）'
    self.description = '立即预约北京地区酒店套餐（2晚，含早餐，下周一入住）'
    ...
    def prepare
      ...
      # 返回给 Agent 的任务信息
      {
        task: "请立即预约#{@city}地区的酒店套餐（#{@night_count}晚，1份），入住日期：下周一（#{@check_in_date.strftime('%Y年%m月%d日')}）开始，连住#{@night_count}晚，请选择包含早餐的套餐选项",
        city: @city,
        night_count: @night_count,
        quantity: @quantity,
        check_in_date: @check_in_date.to_s,
        check_out_date: @check_out_date.to_s,
        hint: "立即预约模式需要选择具体的酒店和入住日期。下周一是#{@check_in_date.strftime('%m月%d日')}。请选择含早餐的套餐选项（含早套餐或豪华套餐）以获得更好的体验。",
    ...
    def verify
      ...
      add_assertion "入住日期正确（下周一开始，连住#{@night_count}晚）", weight: 15 do
        ...
        expect(actual_check_in).to eq(@check_in_date),
          "入住日期错误。期望: #{@check_in_date.strftime('%Y年%m月%d日')}（下周一）, 实际: #{actual_check_in&.strftime('%Y年%m月%d日')}"
    ...
    # 模拟 AI Agent 操作：立即预约北京地区含早餐的酒店套餐（下周一入住）
    def simulate
      # 1. 查找测试用户（数据包中已创建）
      user = User.find_by!(email: 'demo@travel01.com', data_version: 0)
