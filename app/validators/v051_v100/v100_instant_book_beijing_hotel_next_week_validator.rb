...
#   选择立即预约模式，选择包含早餐的套餐选项，
#   并指定下周一入住，连住2晚
# 
# 复杂度分析:
#   1. 需要搜索"北京"地区的酒店套餐（从多个城市中筛选）
#   2. 需要选择2晚的套餐（筛选night_count）
#   3. 需要选择立即预约模式（instant booking）而非囤货模式（stockup）
#   4. 需要从套餐选项中选择含早餐的选项
#   5. 需要计算下周一的日期并设置为入住日期
#   6. 需要选择具体的酒店和填写联系人信息
#   ❌ 不能一次性提供：需要先搜索套餐→计算日期→选择酒店→选择日期→预约
# 
# 评分标准:
#   - 订单已创建 (20分)
#   - 城市正确（北京）(10分)
...
module V051V100
  class V100InstantBookBeijingHotelNextWeekValidator < BaseValidator
    ...
    self.title = '立即预约北京地区酒店套餐（下周一入住，2晚，含早餐）'
    self.description = '需要搜索北京地区的2晚酒店套餐，选择立即预约模式，从套餐选项中选择含早餐的选项，并指定下周一开始入住2晚'
