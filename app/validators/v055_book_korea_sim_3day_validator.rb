...
# 验证用例: 购买韩国3天5GB/天SIM卡（数量1张）
# 
# 任务描述:
#   Agent 需要在系统中搜索韩国地区的SIM卡，
#   找到有效期为3天且每日流量5GB的产品，购买数量1张并成功创建订单
# 
# 评分标准:
#   - 订单已创建 (15分)
#   - 订单类型正确（SIM卡） (15分)
...
class V055BookKoreaSim3dayValidator < BaseValidator
  ...
  def prepare
    ...
    # 返回给 Agent 的任务信息
    {
      task: "请购买一张韩国3天每日5GB流量的SIM卡（数量1张）",
      region: @region,
      validity_days: @validity_days,
      data_requirement: "5GB/天",
      quantity: @quantity,
  ...
  def verify
    ...
    # 断言4: 有效期正确（3天）
    add_assertion "有效期正确（3天）", weight: 15 do
    ...
    # 断言5: 流量正确（包含"5GB/天"关键词）
    add_assertion "流量正确（5GB/天）", weight: 15 do
      sim_card = @order.orderable
  ...
  # 模拟 AI Agent 操作：购买韩国3天5GB/天SIM卡
  def simulate
    # 1. 查找测试用户（数据包中已创建）
