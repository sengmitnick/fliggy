...
class V061BookKoreaWifi5gValidator < BaseValidator
  ...
  def prepare
    ...
    # 返回给 Agent 的任务信息
    {
      task: "请预订一台韩国5G高速WiFi，租用5天",
      region: @region,
      rental_days: @rental_days,
      quantity: @quantity,
      wifi_type: @wifi_keyword,
      hint: "韩国有多款WiFi可选，选择5G高速版（26元/天）。租赁: 1台×5天+押金500=630元。取件: 3天后、深圳市南山区科技园南区深南大道9988号、邮寄。联系人: 赵六/13600136000",
  ...
  def verify
    ...
    # 断言4: 选择了韩国5G高速WiFi
    add_assertion "选择了韩国5G高速WiFi", weight: 25 do
    ...
    # 断言5: 租赁天数和总价正确
    add_assertion "租赁天数正确（5天）、总价正确（630元含押金）", weight: 25 do
      wifi = @order.orderable
