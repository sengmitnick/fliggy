...
# 验证用例121: 帮张三预订明天上午06:00从陆家嘴金融区到浦东机场T2航站楼的送机服务（经济5座，选择同车型最低价）
#
# 任务描述:
#   张三明天上午需要从陆家嘴金融区前往浦东国际机场T2航站楼（搭乘07:00航班）。
#   预订送机服务，出发时间为06:00，选择经济5座车型。
#   Agent 需要创建1个送机订单，明确上车地点、目的地机场航站楼、出发时间，并选择该车型最低价格套餐。
#
# 业务流程（6个关键步骤）：
#   1. 明确送机场景（to_airport = 从指定地点送到机场）
#   2. 确认上车地点（陆家嘴金融区）
#   3. 确认目的地机场和航站楼（浦东国际机场T2航站楼）
#   4. 确认出发时间（06:00）
#   5. 筛选经济5座车型套餐
#   6. 对比同车型所有套餐价格，选择最低价格套餐
#
# 复杂度分析（6个关键点）：
#   1. 需要理解"送机"含义：to_airport = 从指定地点出发，送到机场
#   2. 需要明确上车地点（陆家嘴金融区）作为送机起点
#   3. 需要明确目的地（浦东国际机场T2航站楼）作为送机终点
#   4. 需要确认出发时间（06:00）
#   5. 需要筛选经济5座车型，对比不同供应商的价格
#   6. 需要选择该车型中最低价格套餐
#   ❌ 不能一次性提供：需要先确认上车地点→目的地机场及航站楼→确认出发时间→筛选5座车型→对比价格→预订最低价套餐
#
# 评分标准（6项，总计100分）：
#   - 创建了送机订单（25分）
#   - 乘客信息正确（张三）（10分）
#   - 上车地点正确（陆家嘴金融区）（15分）
#   - 目的地正确（浦东国际机场T2航站楼）（15分）
#   - 出发时间正确（06:00）（10分）
#   - 价格选择合理（该车型最低价格套餐）（25分）
#
# 使用方法:
#   # 准备阶段
...
module V101V150
  class V121BookAirportDropoffValidator < BaseValidator
    ...
    self.title = '帮张三预订明天上午06:00从陆家嘴金融区到浦东机场T2航站楼的送机服务（经济5座，选择同车型最低价）'
    self.description = '帮张三预订明天上午06:00从陆家嘴金融区到浦东机场T2航站楼的送机服务（经济5座，选择同车型最低价）'
    ...
    def prepare
    ...
    
      {
        ...
        hint: "送机服务需要明确上车地点和目的地机场及航站楼。出发时间为06:00",
        statistics: {
    ...
    def verify
      ...
      add_assertion "价格选择合理（该车型最低价）", weight: 25 do
