...
module V101V150
  ...
  self.title = '预订3天后上海→深圳航班+深圳宝安机场附近酒店'
  self.description = '预订3天后从上海到深圳的航班，并预订深圳宝安国际机场附近的酒店（当天入住2晚）'
  ...
  def prepare
    ...
    {
      task: "请预订#{@flight_date.strftime('%Y年%m月%d日')}（3天后）从#{@departure_city}到#{@arrival_city}的航班，并预订#{@hotel_city}宝安国际机场附近的酒店，#{@check_in_date.strftime('%Y年%m月%d日')}入住，住2晚。",
      requirements: {
        departure_city: @departure_city,
