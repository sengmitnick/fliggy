...
# 验证用例110: 帮张三在会员商城买热门商品（里程+现金混合支付，50里程预算，配送到北京朝阳区）
#
# 任务描述:
#   用户张三有50里程预算，需要在会员商城购买1件热门商品。
#   要求使用里程+现金混合支付方式，里程不能超过50。
#   配送地址为北京市朝阳区。
#
# 业务流程（4个关键步骤）：
#   1. 浏览会员商城，筛选热门商品分类（category='popular'）
#   2. 筛选符合预算的商品（里程≤50，且同时需要里程+现金）
#   3. 选择合适的商品，填写配送信息（北京市朝阳区）
#   4. 使用里程+现金混合支付，提交订单
#
# 复杂度分析（5个关键点）：
#   1. 需要理解商品分类：category='popular'（热门商品）
#   2. 需要理解支付方式：里程+现金混合支付（price_mileage > 0 AND price_cash > 0）
#   3. 需要理解预算约束：里程≤50（price_mileage <= 50）
#   4. 需要理解配送信息：配送到北京市朝阳区（需包含联系人、电话、详细地址）
#   5. 需要理解自主选择：在符合条件的商品中自主选择（不指定具体商品）
#   ❌ 不能随机选择：必须精确匹配商品分类、支付方式、预算约束
#
# 评分标准（5项，总计100分）：
#   - 订单已创建（25分）
#   - 商品分类正确（热门商品 category='popular'）（15分）
#   - 使用了混合支付方式（里程+现金）（20分）
#   - 里程使用在预算内（≤50里程）（20分）
#   - 配送信息完整（北京市朝阳区）（20分）
module V101V150
  class V110MembershipMallPurchaseValidator < BaseValidator
    ...
    self.title = '帮张三在会员商城买热门商品（里程+现金混合支付，50里程预算，配送到北京朝阳区）'
    self.description = '帮张三在会员商城买热门商品（里程+现金混合支付，50里程预算，配送到北京朝阳区）'
