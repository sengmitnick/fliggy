...
# 验证用例136: 帮张三预订后天北京→天津最低价火车票（二等座）+天津经济型酒店（后天入住1晚），要求总价最低
#
# 任务描述:
#   张三后天需要从北京到天津，希望预订火车票（二等座）和经济型酒店住1晚。
#   由于预算有限，希望火车票和酒店的总价最低。
#   Agent 需要创建1个火车票订单和1个酒店订单，确保火车票为最低价，酒店为经济型（≤300元），且火车票+酒店总价最低。
#
# 业务流程（9个关键步骤）：
#   1. 明确受益人信息（张三，使用其姓名、身份证号、电话作为乘客和入住人信息）
#   2. 搜索北京→天津火车票（后天出发）
#   3. 按二等座价格升序排序，找到最低价火车票
#   4. 创建火车票订单（座位类型=二等座）
#   5. 搜索天津经济型酒店（price≤300元）
#   6. 筛选酒店房间（room_category='overnight'，排除钟点房）
#   7. 按房间价格升序排序，找到最低价酒店房间
#   8. 创建酒店订单（入住日期=火车日期，入住1晚）
#   9. 验证总价优化（火车票+酒店总价≤理论最低总价的115%）
#
# 复杂度分析（9个关键点）：
#   1. 需要理解火车票+酒店组合预订场景
#   2. 需要明确火车路线（北京→天津，后天出发）
#   3. 需要筛选经济型酒店（hotel.price≤300元）
#   4. 需要理解"总价最低"优化目标（火车票价格+酒店房间价格的总和最小）
#   5. 需要选择二等座座位类型（seat_type = 'second_class'）
#   6. 需要查询最低价火车票（按price_second_class升序排序，取first）
#   7. 需要查询最低价酒店房间（按room.price升序排序，取first，room_category='overnight'）
#   8. 需要使用受益人信息作为乘客和入住人信息
#   9. 需要验证入住日期与火车日期一致（check_in_date = train_date）
#   ❌ 不能一次性提供所有信息：需要分别查询火车和酒店数据，计算总价，选择最优组合，分步骤创建订单。
#
# 评分标准（9项，总计100分）：
#   1. 创建了火车票订单（20分）
#   2. 火车票为最低价（≤60元）（15分）
#   3. 乘客信息正确（张三的姓名和身份证号）（10分）
#   4. 创建了酒店订单（10分）
#   5. 酒店为经济型（price≤300元）（15分）
#   6. 酒店城市正确（天津）（5分）
#   7. 总价优化（火车+酒店总价最低）（15分）
#   8. 入住日期=火车日期（5分）
#   9. 入住人信息正确（张三的姓名和联系电话）（5分）
#
# 使用方法:
#   rake validator:simulate_single[v136_book_cheapest_train_and_budget_hotel_validator]
...
module V101V150
  class V136BookCheapestTrainAndBudgetHotelValidator < BaseValidator
    ...
    self.title = '帮张三预订后天北京→天津最低价火车票（二等座）+天津经济型酒店（后天入住1晚），要求总价最低'
    self.description = '帮张三预订后天北京→天津最低价火车票（二等座）+天津经济型酒店（后天入住1晚），要求总价最低'
