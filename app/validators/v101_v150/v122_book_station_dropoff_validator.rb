...
# 验证用例122: 帮李四预订后天14:00从外滩到上海虹桥站的送站服务（经济5座，选择同车型最低价）
#
# 任务描述:
#   李四后天下午需要从外滩前往上海虹桥火车站乘车。
#   预订送站服务，出发时间为14:00，选择经济5座车型。
#   Agent 需要创建1个送站订单，明确上车地点、目的地火车站、出发时间，并选择该车型最低价格套餐。
#
# 业务流程（6个关键步骤）：
#   1. 明确送站场景（to_station = 从指定地点送到火车站）
#   2. 确认上车地点（外滩）
#   3. 确认目的地火车站（上海虹桥站）
#   4. 确认出发时间（14:00）
#   5. 筛选经济5座车型套餐
...
#
# 复杂度分析（6个关键点）：
#   1. 需要理解"送站"含义：to_station = 从指定地点出发，送到火车站
#   2. 需要明确上车地点（外滩）作为送站起点
#   3. 需要明确目的地（上海虹桥站）作为送站终点
#   4. 需要确认出发时间（14:00）
#   5. 需要筛选经济5座车型，对比不同供应商的价格
#   6. 需要选择该车型中最低价格套餐
#   ❌ 不能一次性提供：需要先确认上车地点→目的地火车站→确认出发时间→筛选5座车型→对比价格→预订最低价套餐
#
# 评分标准（6项，总计100分）：
#   - 创建了送站订单（25分）
#   - 乘客信息正确（李四）（10分）
...
module V101V150
  class V122BookStationDropoffValidator < BaseValidator
    ...
    self.title = '帮李四预订后天14:00从外滩到上海虹桥站的送站服务（经济5座，选择同车型最低价）'
    self.description = '帮李四预订后天14:00从外滩到上海虹桥站的送站服务（经济5座，选择同车型最低价）'
