...
# 验证用例114: 预订地中海邮轮（地中海辉煌号，7天6晚，4月出发，海景房）
#
# 测试内容：
...
class V114BookBarcelonaMediterraneanCruiseOceanviewValidator < BaseValidator
  def initialize
    ...
    @title = '预订地中海邮轮（地中海辉煌号，7天6晚，4月出发，海景房）'
    @description = '预订地中海邮轮航线，选择地中海辉煌号4月份最近一班7天6晚行程，预订海景房（观景之选），为2位成人'
    
    # 核心参数
    @ship_keyword = '辉煌'                # 船只关键词（地中海辉煌号）
    @departure_port_keyword = '巴塞罗那'  # 出发港关键词
    @expected_days = 7                    # 预期天数（7天6晚）
    @expected_nights = 6                  # 预期晚数
    @expected_cabin_category = 'oceanview' # 预期舱房类型（海景房）
  ...
  def verify
    ...
    add_assertion "选择最近可用日期（4月份最早班次）", weight: 10 do
      # 查询所有符合条件的航次（同一船只、同一出发港、相同天数、1月份）
