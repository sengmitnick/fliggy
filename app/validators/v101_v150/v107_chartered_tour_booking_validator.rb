...
class V107CharteredTourBookingValidator < BaseValidator
  ...
  self.description = '预订上海定制游，选择经典路线，预订8小时包车服务，选择7座商务车'
  self.timeout_seconds = 240
  
  def prepare
    @city_name = '上海'
    @route_category = 'classic'
    @duration_hours = 8
    @vehicle_seats = 7
    @passengers_count = 4
  ...
  def verify
    ...
    add_assertion "路线正确（上海经典路线）", weight: 15 do
    ...
    add_assertion "乘客数量正确（4人）", weight: 10 do
      expect(@booking.passengers_count).to eq(@passengers_count),
        "乘客数量错误。期望: #{@passengers_count}人, 实际: #{@booking.passengers_count}人"
    end
