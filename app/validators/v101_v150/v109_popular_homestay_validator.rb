...
# 验证用例109: 帮王芳在成都宽窄巷子订网红民宿（5天后入住，3晚，评分最高且>=4.5分）
#
# 任务描述:
#   用户王芳想在成都宽窄巷子地区预订网红民宿，入住3晚，5天后入住。
#   要求选择评分最高且>=4.5分的民宿（网红民宿通常评分高）。
#   Agent 需要在符合条件的民宿中，选择评分最高的完成预订。
#
# 业务流程（4个关键步骤）：
#   1. 搜索成都宽窄巷子的民宿（hotel_type='homestay'）
#   2. 筛选评分>=4.5分的民宿
#   3. 按评分排序，选择评分最高的民宿（网红民宿）
#   4. 设置入住日期（5天后）、入住天数（3晚），填写王芳的信息并提交订单
#
# 复杂度分析（5个关键点）：
#   1. 需要理解住宿类型：hotel_type='homestay'（民宿，非酒店 'hotel'）
#   2. 需要理解评分筛选：rating >= 4.5（高评分民宿）
#   3. 需要理解评分优化：选择评分最高的民宿（网红民宿通常评分高）
#   4. 需要理解地区筛选：成都宽窄巷子（地址包含'宽窄巷子'）
#   5. 需要理解联系人信息：使用王芳的乘客信息（姓名+手机号）
#   ❌ 不能随机选择：必须精确匹配住宿类型、地区、评分要求，并选择最高评分
#
# 评分标准（7项，总计100分）：
#   - 订单已创建（25分）
#   - 城市/地区正确（成都宽窄巷子）（10分）
#   - 住宿类型正确（民宿 hotel_type='homestay'）（15分）
#   - 入住天数正确（3晚）（10分）
#   - 评分符合要求（>=4.5分）（10分）
#   - 入住人信息正确（王芳 13700137001）（10分）
#   - 选择评分最高的网红民宿（20分）
module V101V150
  class V109PopularHomestayValidator < BaseValidator
    ...
    def prepare
      ...
      @expected_guest_name = @wangfang.name
      @expected_guest_phone = @wangfang.phone
    ...
    def verify
      ...
      # 断言6: 入住人信息正确（权重10%）
      add_assertion "入住人信息正确（王芳 13700137001）", weight: 10 do
      ...
      # 断言7: 选择了评分最高的网红民宿（权重20%）
      add_assertion "选择了评分最高的网红民宿", weight: 20 do
        # 获取所有符合条件的民宿（评分>=4.5）
        qualified_homestays = Hotel.where(
          hotel_type: 'homestay',
          data_version: 0
    ...
    def restore_from_state(data)
      ...
      @expected_guest_name = data['expected_guest_name'] || '王芳'
      @expected_guest_phone = data['expected_guest_phone'] || '13700137001'
