...
# 使用方法:
#   # 准备阶段
#   POST /api/tasks/v115_book_flight_and_airport_pickup_comfort_validator/start
#   
#   # Agent 通过界面操作完成任务...
#   
#   # 验证结果
...
module V101V150
  class V115BookFlightAndAirportPickupComfortValidator < BaseValidator
    self.validator_id = 'v115_book_flight_and_airport_pickup_comfort_validator'
    ...
    def prepare
      ...
      {
        task: "请预订#{@flight_date.strftime('%Y年%m月%d日')}从#{@departure_city}到#{@arrival_city}的航班（到达首都国际机场T3航站楼），" \
    ...
    def verify
      ...
      # 断言6: 车型和价格选择合理（舒适5座，最优选择）(20%)
      add_assertion "车型和价格选择合理（舒适5座，最优选择）", weight: 20 do
        # 验证车型
    ...
    def simulate
      ...
      flight_booking = Booking.create!(
        ...
        passenger_name: '李四',
        passenger_id_number: '440101198501011234',
