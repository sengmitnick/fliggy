...
# 验证用例101: 给张三囤货深圳地区酒店套餐（2晚，2份，含早套餐）
# 
# 任务描述:
#   用户想囤货购买深圳地区的酒店套餐，购买2份2晚套餐，先囤货后预约。
#   要求从套餐的多个选项中选择含早套餐选项（含早套餐或豪华套餐），而非标准套餐。
#   Agent 需要搜索符合条件的套餐，选择含早选项，修改购买数量为2份，并正确计算订单总价完成囤货购买。
# 
# 业务流程（6个关键步骤）：
#   1. 搜索深圳地区的酒店套餐产品
#   2. 筛选2晚的套餐（night_count=2）
#   3. 查看该套餐的多个选项，选择含早套餐选项（含早套餐或豪华套餐）
#   4. 使用囤货模式（stockup），不需指定入住日期
#   5. 修改购买数量为2份（而非默认1份）
#   6. 填写联系人信息，验证订单总价（单价×2份）并提交订单
# 
# 复杂度分析（6个关键点）：
#   1. 需要理解城市筛选：深圳地区的酒店套餐
#   2. 需要理解套餐晚数：2晚（night_count=2）
#   3. 需要理解预约模式：stockup（囤货，先购买后预约，不需指定入住日期）vs instant（立即预约，需指定日期）
#   4. 需要理解套餐选项差异：对比标准套餐和含早套餐的服务内容
#   5. 需要理解数量修改：默认1份，需修改为2份
#   6. 需要理解价格计算：订单总价 = 单价 × 2份
#   ❌ 不能随机选择：必须精确选择含早套餐选项、正确修改数量、正确计算总价
# 
# 评分标准（7项，总计100分）：
#   - 订单已创建（20分）
#   - 城市正确（深圳）（10分）
...
module V101V150
  class V101BookShenzhenHotelMultiplePackagesValidator < BaseValidator
    ...
    self.title = '给张三囤货深圳地区酒店套餐（2晚，2份，含早套餐）'
    self.description = '囤货深圳地区酒店套餐（2晚，2份，含早套餐）'
