...
# 验证用例313: 预订杭州西湖自行车环湖骑行（张三、李四、刘强，5天后，3人，景区门票+1辆双人车+1辆单人车）
#
# 任务描述:
#   张三、李四、刘强预订杭州西湖的自行车环湖骑行服务。
#   要求：5天后，3人，包含景区门票（游船票）和自行车租赁（1辆双人车+1辆单人车）。
#   Agent 需要创建三个订单：
#   1) 景区门票订单（TicketOrder）- 西湖游船票，3人
#   2) 自行车租赁订单（ActivityOrder）- 双人车，2个座位
#   3) 自行车租赁订单（ActivityOrder）- 单人车，1个座位
#   联系人使用张三、李四或刘强的信息。
#
# 业务流程（7个关键步骤）：
#   1. 搜索杭州西湖景点
...
#   4. 查找单人自行车租赁活动
#   5. 确定游玩日期（5天后）和人数（3人）
#   6. 创建门票订单（3人）和两个自行车租赁订单（双人车+单人车）
#   7. 确保所有订单使用相同的联系人、日期，游客信息正确分配
#
# 复杂度分析（6个关键点）：
#   1. 需要理解自行车环湖骑行的服务组合：门票+双人车+单人车
#   2. 需要创建三个订单（1个TicketOrder + 2个ActivityOrder，不同自行车类型）
#   3. 需要计算正确的游玩日期（5天后）
#   4. 需要选择demo用户的乘客（张三、李四、刘强中任意1人）作为联系人
#   5. 需要正确分配游客：门票3人、双人车2人、单人车1人
#   6. 需要区分双人车和单人车两种不同的自行车类型
#
# 评分标准（12项，总计100%）：
#   - 创建了景点门票订单 (14%)
#   - 景点正确（杭州西湖） (9%)
...
module V301V350
  class V313BookBicycleTourRoutePlanningLogisticsValidator < BaseValidator
    ...
    self.title = '预订杭州西湖自行车环湖骑行（张三、李四、刘强，5天后，3人，景区门票+1辆双人车+1辆单人车）'
    self.description = '预订杭州西湖的自行车环湖骑行服务，张三、李四、刘强，5天后，3人，要景区门票（游船票）和自行车租赁（1辆双人车+1辆单人车）'
