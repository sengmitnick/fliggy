  ...
  class V315BookRaftingAdventureSafetyEquipmentValidator < BaseValidator
    ...
    def prepare
      ...
      @attraction = Attraction
        .where(name: '长江索道', data_version: 0)
        .first
      
      raise "未找到长江索道景点" unless @attraction
      ...
      raise "未找到长江索道的漂流体验活动" unless @rafting_activity
      
      {
        task: "请预订长江索道漂流体验活动（#{@activity_date.strftime('%Y年%m月%d日')}，#{@participant_count}人），包含安全保障和全套装备。",
        requirements: {
          attraction: '长江索道',
        ...
        hint: "需要预订长江索道漂流体验活动，并购买保险确保安全。"
      }
    end
    
    def verify
      add_assertion "创建了长江索道漂流体验活动订单", weight: 40 do
        ...
        expect(@rafting_order).not_to be_nil, "未找到长江索道漂流体验活动订单"
      end
      ...
      add_assertion "活动包含保险（安全保障）", weight: 30 do
        ...
        expect(insurance_type).not_to eq('none'),
          "漂流体验活动未购买保险。期望: basic或premium，实际: #{insurance_type}"
      end
      
      add_assertion "活动日期正确", weight: 20 do
        expect(@rafting_order.visit_date).to eq(@activity_date),
          "漂流体验活动日期错误。期望: #{@activity_date}，实际: #{@rafting_order.visit_date}"
      end
