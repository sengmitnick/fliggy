...
module V001V050
  class V020BookHotelShenzhenTwoNightsValidator < BaseValidator
    ...
    self.title = '预订明天深圳酒店（1间房1成人，入住2晚）'
    ...
    def verify
      ...
      # 断言3: 入住日期正确
      add_assertion "入住日期正确（明天）", weight: 15 do
        expect(@hotel_booking.check_in_date).to eq(@check_in_date),
          "入住日期错误。期望: #{@check_in_date}, 实际: #{@hotel_booking.check_in_date}"
      end
    
      # 断言4: 离店日期正确（核心评分项）
      add_assertion "离店日期正确（大后天，共2晚）", weight: 25 do
      ...
      # 断言5: 房间数和人数正确
      add_assertion "房间数和人数正确（1间房，1成人，0儿童）", weight: 20 do
