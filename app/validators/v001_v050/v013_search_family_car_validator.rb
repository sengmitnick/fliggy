...
# 验证用例: 给张三（13800138000）在成都天府广场租车点租一辆适合家庭的7座SUV（后天上午9:00取车，租3天）
# 
# 任务描述:
#   Agent 需要为张三（13800138000）在成都天府广场租车点搜索租车服务，
...
module V001V050
  class V013SearchFamilyCarValidator < BaseValidator
    ...
    self.title = '给张三（13800138000）在成都天府广场租车点租一辆适合家庭的7座SUV（后天上午9:00取车，租3天）'
    self.description = '给张三（13800138000）在成都天府广场租车点租一辆7座SUV适合家庭出行（后天上午9:00取车，租3天）'
    ...
    def prepare
      ...
      # 返回给 Agent 的任务信息
      {
        ...
        task_detail: "请在#{@location}#{@pickup_location}搜索适合家庭的#{@required_seats}座#{@category}并租赁（后天上午#{@pickup_time}取车，租#{@rental_days}天）",
        location: @location,
        pickup_location: @pickup_location,
        category: @category,
        seats: @required_seats,
        ...
        hint: "适合家庭的车辆通常是7座SUV，空间大、舒适度高。取车地点：#{@pickup_location}",
    ...
    def verify
      # 断言1: 必须有订单创建（查询过滤核心实体：城市+取车地点+车型）
      add_assertion "创建了租车订单", weight: 15 do
        ...
        expect(all_orders).not_to be_empty, "未找到任何#{@location}#{@pickup_location}#{@category}的租车订单记录"
      ...
      # 断言3: 取车地点正确
      add_assertion "取车地点正确（天府广场租车点）", weight: 10 do
        expect(@order.car.pickup_location).to eq(@pickup_location),
          "取车地点不正确。期望: #{@pickup_location}, 实际: #{@order.car.pickup_location}"
      end
    
      # 断言4: 驾驶员信息正确（验证来自 demo_user，不是硬编码）
      ...
      # 断言6: 座位数正确（核心评分）
      add_assertion "座位数正确（7座）", weight: 15 do
      ...
      add_assertion "租期天数正确（#{@rental_days}天）", weight: 15 do
        # 计算租期天数：还车日期 - 取车日期（不加1，与前端保持一致）
        # 例如: 27号9点到 2号9点 = 3天 (72小时)
        actual_rental_days = (@order.return_datetime.to_date - @order.pickup_datetime.to_date).to_i
    ...
    def simulate
      ...
      # 3天租期：27号上午9点 -> 3朁30日上午9点（恰好72小时=3天）
      # 不加1: return_date = pickup_date + 3.days
