...
# 验证用例: 购买日本5天10GB/天流量SIM卡（数量1张）
# 
# 任务描述:
#   Agent 需要在系统中搜索日本地区的SIM卡，
#   找到有效期为5天且流量为10GB/天的产品，购买数量1张并成功创建订单
# 
# 评分标准:
#   - 订单已创建 (15分)
#   - 订单类型正确（SIM卡） (15分)
...
class V054BookJapanSim5dayValidator < BaseValidator
  ...
  self.title = '购买日本5天10GB/天流量SIM卡（数量1张）'
  self.description = '搜索日本地区的SIM卡，找到5天有效期且流量为10GB/天的产品并购买1张'
  ...
  def prepare
    ...
    # 返回给 Agent 的任务信息
    {
      task: "请购买一张日本5天10GB/天流量的SIM卡（数量1张）",
      region: @region,
      validity_days: @validity_days,
  ...
  # 模拟 AI Agent 操作：购买日本5天10GB/天流量SIM卡
  def simulate
    # 1. 查找测试用户（数据包中已创建）
