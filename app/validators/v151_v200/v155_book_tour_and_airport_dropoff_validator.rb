...
# 验证用例155: 给张三订明天上海2日跟团游，并订机场送机服务（后天早8:30从陆家嘴金融区接送服务点出发去浦东T2）
#
# 任务描述:
#   张三计划明天参加上海2日跟团游，并预订后天机场送机服务从陆家嘴金融区接送服务点去浦东T2航站楼，后天早上8:30出发。
#   1. 上海2日跟团游（明天出发）
#   2. 机场送机服务（从陆家嘴金融区接送服务点接送至浦东T2航站楼，后天早上8:30出发）
#
# 任务分解步骤:
#   1. 查询上海2日跟团游产品（destination=上海，duration=2，travel_type=跟团游）
#   2. 创建跟团游订单（travel_date=明天，乘客=张三）
#   3. 查询Flight获取后天从上海浦东出发的航班，获取出发机场信息（含航站楼，如浦东T2）（送达地点）
#   4. 查询TransferLocation获取上海陆家嘴金融区接送服务点（location_type='other'，名称包含'陆家嘴'）（出发地点）
#   5. 创建送机服务订单（transfer_type=airport_dropoff，location_from=陆家嘴金融区接送服务点，location_to=浦东T2，pickup_datetime=后天早上8:30）
#
# 复杂度分析（5个复杂点）：
#   1. 多模块组合：需要同时创建跟团游订单+送机服务订单（2个不同类型的订单）
#   2. 出发地点查询：需要从 TransferLocation 查询具体的出发地点（陆家嘴金融区接送服务点，不使用笼统的"市区"）
...
#   3. 出发日期正确=明天（10分）
#   4. 创建了送机服务（20分）
#   5. 出发地点正确=陆家嘴金融区接送服务点（从 TransferLocation 动态获取）（15分）
...
module V151V200
  class V155BookTourAndAirportDropoffValidator < BaseValidator
    ...
    self.title = '给张三订明天上海2日跟团游，并订机场送机服务（后天早8:30从陆家嘴金融区接送服务点出发去浦东T2）'
    self.description = '给张三订明天上海2日跟团游，并订机场送机服务（后天早上8:30从陆家嘴金融区接送服务点出发去浦东T2航站楼）'
    ...
    def prepare
      ...
      @flight_departure_airport_short = @flight.departure_airport  # 出发机场简称（如：浦东T2）
      
      # 查询TransferLocation获取上海浦东国际机场T2航站楼（送达地点）
      @dropoff_loc = TransferLocation.where(
        city: @city,
        location_type: 'airport',
        data_version: 0
      ).find { |loc| loc.name.include?('浦东') && loc.name.include?('T2') && loc.name.include?('航站楼') }
      
      raise "数据包缺少上海浦东国际机场T2航站楼TransferLocation" unless @dropoff_loc
      
      @flight_departure_airport = @dropoff_loc.name  # 送达地点=浦东国际机场T2航站楼（从TransferLocation动态获取完整名称）
      
      # 查询TransferLocation获取上海陆家嘴金融区接送服务点（出发地点）
      ...
      raise "数据包缺少上海陆家嘴金融区接送服务点TransferLocation" unless @pickup_loc
      
      @pickup_location = @pickup_loc.name  # 出发地点=陆家嘴金融区接送服务点（从TransferLocation动态获取）
      ...
      expect(@available_tours).not_to be_empty, "数据包缺少上海2日跟团游产品"
    ...
    def simulate
      ...
      # 设置送机时间（后天早上8:30）
      pickup_datetime = @flight_date.in_time_zone.change(hour: 8, min: 30)
      ...
      Transfer.create!(
        ...
        location_from: @pickup_location,  # 出发地点=陆家嘴金融区接送服务点（从TransferLocation动态获取）
        location_to: @flight_departure_airport,  # 送达地点=浦东T2（从Flight.departure_airport动态获取）
        pickup_datetime: pickup_datetime,  # 后天早上8:30出发
    ...
    def verify
      ...
      # 断言5: 出发地点正确=陆家嘴金融区接送服务点（从 TransferLocation 动态获取）
      ...
      # 断言7: 出发时间正确=后天早上8:30
      expected_pickup_time = @flight_date.in_time_zone.change(hour: 8, min: 30)
      add_assertion "出发时间正确（后天早上8:30）", weight: 10 do
        actual_time = @transfer.pickup_datetime.in_time_zone
        expected_time = expected_pickup_time
        
        # 比较时忽略时区差异，只比较绝对时间
        expect(actual_time.to_i).to eq(expected_time.to_i),
          "出发时间错误。期望: #{expected_time.strftime('%Y-%m-%d %H:%M %Z')}（后天早上8:30）, 实际: #{actual_time.strftime('%Y-%m-%d %H:%M %Z')}"
      end
    end
  end
end
