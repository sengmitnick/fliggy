...
# 验证用例192: 给李四预订4天后从北京到上海的往返火车+7晚住宿（总预算≤3000元，选择最便宜的火车）
#
# 任务描述：
#   为李四预订4天后从北京到上海的7天行程（往返火车+住宿7晚），要求选择最便宜的火车
#
# 核心要求：
#   - 乘客：李四（1人）
#   - 出发日期：4天后（Date.current + 4.days）
#   - 返程日期：出发后7天（出发日期 + 7.days）
#   - 路线：北京 ⇄ 上海
#   - 预算限制：总价 ≤ 3000元
#   - 住宿：7晚（入住日期=去程到达日期）
#   - 价格策略：选择最便宜的去程火车、返程火车、酒店房间
#
# 业务流程：
#   1. 查询4天后北京→上海的去程火车
#   2. 查询11天后上海→北京的返程火车（去程+7天）
#   3. 查询上海的经济型酒店
#   4. 选择最便宜的去程火车
#   5. 选择最便宜的返程火车
#   6. 选择最便宜的酒店房间
#   7. 创建去程火车票订单
#   8. 创建返程火车票订单
#   9. 创建酒店订单（入住日期=去程到达日期，住7晚）
#
# 复杂度分析：
#   - 数据量：去程火车（10条），返程火车（10条），酒店（20家）
#   - 计算复杂度：需要计算去程+返程+酒店的价格组合
#   - 关键优化：选择单项最便宜（去程最低价+返程最低价+酒店最低价）
#
# 验证要点：
#   - 去程订单：北京→上海，4天后出发，选择最便宜的二等座
#   - 返程订单：上海→北京，11天后出发，选择最便宜的二等座
#   - 酒店订单：上海，7晚，入住日期=去程到达日期，选择最便宜的房间
#   - 总价 ≤ 3000元
#   - 乘客信息：李四
#
# 评分标准：
...
module V151V200
  class V192BookWeekTripBudget3000OptimizeValidator < BaseValidator
    ...
    self.title = '给李四预订4天后从北京到上海的往返火车+7晚住宿（总预算≤3000元，选择最便宜的火车）'
    self.description = '帮李四订4天后从北京到上海的7天行程（往返火车+住宿7晚），总预算不超过3000元，选择最便宜的火车'
    ...
    def prepare
      ...
      
      {
        ...
        hint: "需要预订去程、返程火车和7晚酒店，总价不超过#{@max_budget}元。优先选择最便宜的火车"
      }
    end
