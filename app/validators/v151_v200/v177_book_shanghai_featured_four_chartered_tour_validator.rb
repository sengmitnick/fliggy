...
# 验证用例177: 预订上海精华四景游（商务巴士，团队出行，7天后出发）
#
# 核心验证点:
# 1. 路线选择: 上海精华四景
# 2. 车型选择: 商务巴士（座位数≥12人）
...
module V151V200
  class V177BookShanghaiFeaturedFourCharteredTourValidator < BaseValidator
    self.validator_id = 'v177_book_shanghai_featured_four_chartered_tour_validator'
    self.task_id = 'a692ca4a-a4a8-4060-b7d5-589e7c07a2b2'
    self.title = '预订上海精华四景游（商务巴士，团队12人）- 验证出发日期、包车时长8小时、订单完整性'
    self.description = '预订上海精华四景包车路线，团队12人出行，选择商务巴士车型，8小时服务。验证出发日期（7天后）、包车时长（8小时）、车型座位数、订单信息完整性。'
    ...
    def prepare
      ...
      {
        task: "请预订#{@travel_date.strftime('%Y年%m月%d日')}的#{@city_name}#{@route_keyword}包车路线，团队#{@passenger_count}人出行，选择#{@vehicle_type_name}车型，#{@duration_hours}小时服务",
        city: @city_name,
