...
# 验证用例193: 给王芳预订明天北京到上海的预算内最高等级的航班+酒店（预算2000元内）
#
# 任务描述：
#   为王芳预订明天从北京到上海的航班+酒店，在预算2000元内尽量升级到最高等级（商务舱或头等舱、高星级酒店）
#
# 核心要求：
#   - 乘客：王芳（1人）
#   - 出行日期：明天（Date.current + 1.day）
#   - 路线：北京 → 上海
#   - 预算限制：总价 ≤ 2000元
#   - 住宿：1晚（入住日期=航班到达日期）
#   - 优化目标：预算内选择最高等级（舱位+酒店星级）
#
# 业务流程：
#   1. 查询明天北京→上海的所有航班（经济舱/商务舱/头等舱）
#   2. 查询上海的所有星级酒店
#   3. 遍历所有航班+酒店组合
#   4. 过滤超出预算的组合（航班价格 + 酒店房价 > 2000元）
#   5. 计算每个组合的等级评分（舱位分 + 星级分）
#   6. 选择评分最高的组合
#   7. 创建航班订单
#   8. 创建酒店订单（入住日期=航班到达日期，住1晚）
#
# 复杂度分析：
#   - 数据量：航班（20-30条）× 酒店（50+家） ≈ 1000-1500种组合
#   - 计算复杂度：O(M × N)，需要遍历所有组合并评分
#   - 评分系统：经济舱=1分，商务舱=2分，头等舱=3分 + 酒店星级（3-5分）
#   - 关键优化：预算约束下的贪婪选择（选择总分最高的组合）
#   - 注意事项：必须过滤room_category='overnight'，排除钟点房
#
# 验证要点：
#   - 航班订单：北京→上海，明天出发
#   - 酒店订单：上海，1晚，入住日期=航班到达日期
#   - 总价 ≤ 2000元
#   - 等级评分 ≥ 最佳组合的80%（允许小幅偏差）
#   - 乘客信息：王芳
#
...
module V151V200
  class V193BookPremiumUpgradeWithinBudgetValidator < BaseValidator
    ...
    self.title = '给王芳预订明天北京到上海的预算内最高等级的航班+酒店（预算2000元内）'
    ...
    def prepare
      ...
      {
        task: "请为#{@passenger.name}预订#{@travel_date.strftime('%m月%d日')}从#{@departure_city}到#{@arrival_city}的航班+酒店，预算#{@max_budget}元，尽可能升级到最高等级",
