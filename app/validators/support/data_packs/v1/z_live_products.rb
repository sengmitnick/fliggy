...
tour_group_product = TourGroupProduct.where(data_version: 0)
                                    .where("destination LIKE '%深圳%'")
                                    .where("title LIKE '%欢乐谷%' OR title LIKE '%一日游%'")
                                    .first

# 如果没有找到，就取深圳的第一个产品
tour_group_product ||= TourGroupProduct.where(data_version: 0).where("destination LIKE '%深圳%'").first

# 获取酒店套餐
hotel_package = HotelPackage.where(data_version: 0).first
