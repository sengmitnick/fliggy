...
# 验证用例: 购买美国7天10GB/天SIM卡（数量1张）
# 
# 任务描述:
#   Agent 需要在系统中搜索美国地区的SIM卡，
#   找到有效期为7天且每日流量10GB的产品，购买数量1张并成功创建订单
# 
# 评分标准:
#   - 订单已创建 (15分)
#   - 订单类型正确（SIM卡） (15分)
...
# 使用方法:
#   # 准备阶段
#   POST /api/verify/book_usa_sim_7day/prepare
#   
#   # Agent 通过界面操作完成购买...
#   
#   # 验证结果
...
class V058BookUsaSim7dayValidator < BaseValidator
  ...
  def prepare
    ...
    # 返回给 Agent 的任务信息
    {
      task: "请购买一张美国7天每日10GB流量的SIM卡（数量1张）",
      region: @region,
      validity_days: @validity_days,
  ...
  # 模拟 AI Agent 操作：购买美国7天10GB/天SIM卡
  def simulate
    # 1. 查找测试用户（数据包中已创建）
