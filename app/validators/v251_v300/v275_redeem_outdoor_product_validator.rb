...
module V251V300
  # V275: 帮张三在积分商城的「户外运动」分类中选择销量最高的商品兑换
  #
  # 场景: 用户在户外运动分类中选择商品兑换
  # 考点: 分类筛选、户外用品兑换
  #
  # 业务流程:
  #   1. 用户输入：需要在积分商城的「户外运动」分类中选择商品兑换
  #   2. 商品筛选：在户外运动分类中按销量降序排列商品
  #   3. 商品选择：选择销量最高的户外运动商品
  #   4. 提交订单：创建订单并支付
  #
  # 复杂度分析:
  #   1. **分类筛选**（低）：需要筛选户外运动分类商品
  #   2. **商品选择**（低）：选择销量最高的商品
  #   3. **订单创建**（低）：创建单个订单
  #
  # 评分标准（总分100%）:
  ...
  class V275RedeemOutdoorProductValidator < BaseValidator
    ...
    self.title = '帮张三在积分商城的「户外运动」分类中选择销量最高的商品兑换'
    self.description = '帮张三在积分商城的「户外运动」分类中选择销量最高的商品兑换'
    ...
    def prepare
      ...
      
      {
        ...
        requirements: {
          ...
          outdoor_products: @outdoor_products.map { |p| 
            {
              name: p.name,
              price_mileage: p.price_mileage,
        ...
        hint: "#{@category_name}分类包含登山、露营、徒步等户外装备。建议选择销量最高的商品。"
      }
    end
