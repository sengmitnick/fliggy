...
# 验证用例295: 给刘强和陈静预订6天后西安自由行（2个不同的一日游+配备停车场的酒店住3晚）
#
# 任务描述:
#   给刘强和陈静预订6天后去西安玩3天的自由行，需要预订2个不同的一日游产品，
#   以及有停车场的酒店住3晚（方便自驾出行）。
#   Agent需要创建2个自由行订单并配套预订提供停车场的酒店。
#
# 业务流程（6个关键步骤）：
#   1. 明确受益人信息（刘强和陈静，2成人）
#   2. 搜索西安的一日游自由行产品
#   3. 创建2个不同的自由行订单（使用真实存在的路线）
#   4. 为每个订单添加游客信息（刘强和陈静，包含身份证号）
#   5. 筛选配备停车场的酒店（facilities包含"停车"或"免费停车"）
#   6. 创建酒店订单（入住日期=出行日期，住3晚）
#
# 复杂度分析（7个关键点）：
#   1. 需要创建2个独立的自由行订单，确保选择不同的产品
#   2. 需要为每个订单添加2个游客信息（刘强和陈静）
#   3. 需要验证游客信息完整性（姓名、身份证号匹配）
#   4. 需要理解自驾游场景，选择配备停车场的酒店
#   5. 需要在配备停车场的酒店中选择最优选项（评分最高）
#   6. 需要确保自由行和酒店的城市、日期一致性
#   7. 需要使用真实存在的路线（目的地包含西安，travel_type='自由出行'）
#   ❌ 不能选择不提供停车场的酒店，必须严格满足自驾停车要求
#
# 评分标准（8项，总计100分）：
#   1. 创建了2个自由行订单（20分）
#   2. 自由行目的地正确（西安）（10分）
#   3. 游客信息正确（刘强和陈静，2成人）（15分）
#   4. 创建了酒店预订（20分）
#   5. 酒店在西安且配备停车场（15分）- 核心业务逻辑
#   6. 入住人信息正确（刘强或陈静）（10分）
#   7. 入住日期正确（6天后，住3晚）（5分）
#   8. 订单状态正确（5分）
#
# 使用方法:
#   rake validator:simulate_single[v295]
...
module V251V300
  class V295BookMuslimFriendlyServiceValidator < BaseValidator
    ...
    self.title = '给刘强和陈静预订6天后西安自由行（2个不同的一日游+配备停车场的酒店住3晚）'
    self.description = '给刘强和陈静预订6天后去西安玩3天的自由行，预订2个不同的一日游产品，以及配备停车场的酒店住3晚'
