...
# V261: 帮张三和李四预订泰国5天境外跟团游（15天后出发，2人），并购买境外旅游保险（含医疗、意外、行李等保障）
#
# 任务描述:
#   帮张三和李四预订泰国境外跟团游（15天后出发，5天行程，2人），
#   并购买境外旅游保险（含境外医疗、意外伤害、行李丢失等保障）。
#   境外游必须购买境外旅游保险（product_type: international），保障应涵盖医疗、意外、行李等。
#
# 业务流程:
#   1. 用户输入：目的地（泰国）、出发日期（15天后）、人数（2人：张三、李四）、需求（境外游+保险）
#   2. 系统筛选：显示泰国境外游产品（5天行程），同时展示适配的境外旅游保险
#   3. 用户选择：选择合适的境外游套餐，确认保险天数与旅游天数匹配
#   4. 填写信息：联系人（张三或李四）、出行人（张三、李四）、被保险人（张三、李四）
#   5. 确认支付：核对境外游订单、保险订单信息和总价格
...
#   2. **国际保险类型筛选**（高）：必须识别国际旅游保险（product_type: 'international'），确保境外医疗保障
#   3. **多人投保处理**（中）：2人境外游，保险订单需包含2个被保险人（insured_persons数组），需包含身份证号
#   4. **保险时间覆盖验证**（中）：保险起止日期需完全覆盖旅游出行时间（起始日期≤出发日期，结束日期≥返程日期）
#   5. **出行人与被保险人一致性**（中）：验证被保险人姓名和身份证号与出行人信息完全一致
#
# 评分标准（总分100%）:
#   - 创建了境外游订单 (20%) - 基础操作
#   - 创建了出行人信息（张三、李四）(15%) - 出行人信息完整性
#   - 创建了保险订单 (20%) - 核心操作
#   - 保险类型正确（境外旅游保险 international）(15%) - 核心要求（最高权重之一）
#   - 被保险人与出行人一致（张三、李四）(15%) - 信息一致性验证（含姓名和身份证号）
#   - 保险时间覆盖出行时间 (10%) - 业务逻辑正确性
#   - 订单状态有效 (5%) - 订单可用性
module V251V300
  class V261BookInternationalTravelWithInsuranceValidator < BaseValidator
    ...
    self.title = '帮张三和李四预订泰国5天境外跟团游（15天后出发，2人），并购买境外旅游保险（含医疗、意外、行李等保障）'
    self.description = '帮张三和李四预订泰国境外跟团游（15天后出发，5天行程，2人），并购买境外旅游保险（含境外医疗、意外伤害、行李丢失等保障）'
    ...
    def prepare
      ...
      {
        task: "请帮张三和李四预订#{@destination}境外游（#{@travel_date.strftime('%Y年%m月%d日')}出发，#{@duration}天，2人），并购买境外旅游保险（包含医疗、意外、行李等保障）。",
        requirements: {
