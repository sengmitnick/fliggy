...
# V259: 预订张家口崇礼万龙滑雪场（7天后，1人）：景点门票+滑雪装备租赁活动+包含滑雪场景的运动保险
#
# 任务描述:
#   用户需要预订张家口崇礼万龙滑雪场，游玩日期为7天后，1人。需要购买：1）景点门票（TicketOrder），2）滑雪装备租赁活动（ActivityOrder），3）包含滑雪场景保障的运动保险（InsuranceOrder，保障期至少1天），确保所有订单状态有效
#
# 评分标准:
#   - 创建了景点门票订单（TicketOrder，崇礼万龙滑雪场） (25%)
#   - 游玩日期正确（7天后） (10%)
#   - 人数正确（1人） (5%)
#   - 创建了滑雪装备租赁活动订单（ActivityOrder，滑雪装备租赁） (25%)
...
module V251V300
  class V259BookHighRiskActivityWithInsuranceValidator < BaseValidator
    ...
    self.title = '预订张家口崇礼万龙滑雪场（7天后，1人）：景点门票+滑雪装备租赁活动+包含滑雪场景的运动保险'
    self.description = '用户需要预订张家口崇礼万龙滑雪场，游玩日期为7天后，1人。需要购买：1）景点门票（TicketOrder），2）滑雪装备租赁活动（ActivityOrder），3）包含滑雪场景保障的运动保险（InsuranceOrder，保障期至少1天），确保所有订单状态有效'
    ...
    def prepare
      ...
      {
        task: "请预订#{@city}#{@attraction_name}（#{@visit_date.strftime('%Y年%m月%d日')}，#{@quantity}人），需要购买：1）景点门票，2）#{@activity_name}活动，3）包含滑雪场景保障的运动保险（保障期至少1天）。",
        requirements: {
          city: @city,
    ...
    def verify
      add_assertion "创建了景点门票订单（TicketOrder，#{@attraction_name}）", weight: 25 do
      ...
      add_assertion "游玩日期正确（#{@visit_date.strftime('%Y年%m月%d日')}）", weight: 10 do
      ...
      add_assertion "创建了滑雪装备租赁活动订单（ActivityOrder，#{@activity_name}）", weight: 25 do
