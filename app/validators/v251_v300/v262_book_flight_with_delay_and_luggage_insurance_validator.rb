...
# V262: 帮张三预订3天后从北京到上海的航班（1人），并购买交通意外险（必须包含航班延误保障）
#
# 任务描述:
#   帮张三预订3天后从北京到上海的航班（1人），
#   并购买交通意外险（必须包含航班延误保障，保险日期与航班日期一致）。
#   航班出行建议购买交通意外险（product_type: transport），应包含航班延误保障场景。
#
# 业务流程:
#   1. 用户输入：出发地（北京）、目的地（上海）、出发日期（3天后）、乘客（张三）、需求（航班+延误险）
#   2. 系统筛选：显示北京到上海的航班（3天后出发），同时展示适配的交通意外险（含延误保障）
#   3. 用户选择：选择合适的航班，确认保险日期与航班日期一致
#   4. 填写信息：乘客信息（张三）、联系电话、被保险人（张三）
#   5. 确认支付：核对航班订单、保险订单信息和总价格
#   6. 完成订单：生成2个订单（Booking + InsuranceOrder），保险关联到Booking
#   7. 获取凭证：获取航班订单凭证、保险凭证
#
# 复杂度分析:
#   1. **跨模型订单关联逻辑**（中）：需同时创建Booking（航班）和InsuranceOrder，并建立关联关系
#   2. **交通保险类型筛选**（高）：必须识别交通意外险（product_type: 'transport'），确保航班延误保障
#   3. **保险场景验证**（中）：验证保险产品包含'航班延误保障'或'航空保障'场景
#   4. **保险日期与航班日期匹配**（中）：保险起始日期需与航班出发日期完全一致
#   5. **单人投保处理**（低）：1人航班，保险订单需包含1个被保险人（insured_persons数组），需包含身份证号
#
# 评分标准（总分100%）:
#   - 创建了航班订单 (18%) - 核心操作
#   - 创建了保险订单 (14%) - 核心操作
#   - 航班日期正确（3天后） (14%) - 日期准确性
#   - 用户一致性（航班和保险为同一用户） (9%) - 数据关联正确性
#   - 保险日期与航班日期匹配 (9%) - 业务逻辑正确性
#   - 保险类型正确（交通意外险 transport） (14%) - 核心要求（最高权重之一）
#   - 乘客信息正确（张三） (9%) - 信息完整性
#   - 保险被保险人信息正确（张三） (5%) - 信息一致性验证
#   - 保险包含延误保障 (5%) - 保险场景验证
#   - 订单状态有效 (3%) - 订单可用性
module V251V300
  class V262BookFlightWithDelayAndLuggageInsuranceValidator < BaseValidator
    ...
    self.title = '帮张三预订3天后从北京到上海的航班（1人），并购买交通意外险（必须包含航班延误保障）'
    self.description = '帮张三预订3天后从北京到上海的航班（1人），并购买交通意外险（必须包含航班延误保障，保险日期与航班日期一致）'
