...
# V264: 帮张三、李四、王芳3人在杭州预订明天开始的5天自驾游（明天取车，5天后还车，3人），并购买适合自驾游场景的旅游保险（境内旅游险或交通意外险，3人投保，保险时间与租车时间一致）
#
# 任务描述:
#   帮张三、李四、王芳3人在杭州预订明天开始的5天自驾游（明天取车，5天后还车），
#   并购买适合自驾游场景的旅游保险（境内旅游险或交通意外险）。
#   保险需为3人投保，保险时间需与租车时间一致，保障天数覆盖整个租车期间（5天）。
#   自驾游场景优先选择境内旅游险（含自驾游场景），其次可选择交通意外险。
#
# 业务流程:
#   1. 用户输入：目的地（杭州）、取车日期（明天）、租车天数（5天）、人数（3人：张三、李四、王芳）、需求（自驾游+保险）
#   2. 系统筛选：显示杭州租车产品（明天可取车），同时展示适配的自驾游保险（境内旅游险或交通意外险，5天保障）
#   3. 用户选择：选择合适的租车产品，确认保险天数与租车天数匹配
#   4. 填写信息：驾驶员信息（张三/李四/王芳任一）、联系电话、被保险人（张三、李四、王芳3人）
#   5. 确认支付：核对租车订单、保险订单信息和总价格
#   6. 完成订单：生成2个订单（CarOrder + InsuranceOrder），保险关联到CarOrder
#   7. 获取凭证：获取租车订单凭证、保险凭证
#
# 复杂度分析:
#   1. **跨模型订单关联逻辑**（中）：需同时创建CarOrder和InsuranceOrder，并建立关联关系
#   2. **自驾游保险类型筛选**（高）：必须识别适合自驾游场景的保险（境内旅游险含自驾游场景 或 交通意外险），优先选择境内旅游险
#   3. **多人投保处理**（中）：3人自驾游，保险订单需包含3个被保险人（insured_persons数组），需包含身份证号
#   4. **保险时间覆盖验证**（中）：保险起始日期≤取车日期，保险结束日期≥还车日期，保险天数覆盖租车天数（5天）
#   5. **驾驶员与被保险人关系**（低）：验证驾驶员是被保险人之一，3个被保险人姓名和身份证号正确
#
# 评分标准（总分100%）:
#   - 创建了租车订单 (25%) - 基础操作（最高权重之一）
#   - 创建了保险订单 (25%) - 核心操作（最高权重之一）
#   - 保险类型适合自驾游 (15%) - 核心要求（境内旅游险含自驾游场景 或 交通意外险）
#   - 联系人信息正确（张三/李四/王芳任一） (10%) - 驾驶员信息完整性
#   - 保险人数正确（3人） (10%) - 投保人数准确性
#   - 被保险人信息正确（张三、李四、王芳） (5%) - 信息一致性验证（含姓名和身份证号）
#   - 保险时间覆盖租车时间 (10%) - 业务逻辑正确性（起止日期和天数验证）
module V251V300
  class V264BookSelfDriveWithComprehensiveInsuranceValidator < BaseValidator
    ...
    self.title = '帮张三、李四、王芳3人在杭州预订明天开始的5天自驾游（明天取车，5天后还车，3人），并购买适合自驾游场景的旅游保险（境内旅游险或交通意外险，3人投保，保险时间与租车时间一致）'
    ...
    def verify
      ...
      add_assertion "保险类型适合自驾游", weight: 15 do
        ...
        # 境内旅游险或交通意外险都适合自驾游场景
        is_suitable = ['domestic', 'transport'].include?(product_type)
        
        expect(is_suitable).to be_truthy,
          "保险类型不适合自驾游。产品类型: #{product_type}，场景: #{scenes.inspect}。自驾游需要境内旅游险（domestic）或交通意外险（transport）"
      end
