...
# V260: 帮张建国（老年人）预订桂林跟团游（10天后出发，行程≤5天，1位老年人），并购买境内旅游保险（保险类型：境内旅游保险，保额：意外身故≥50万、意外医疗≥5万，保险日期与跟团游匹配，保险天数与行程天数一致）
#
# 任务描述:
#   帮张建国（老年人）预订桂林跟团游（10天后出发，行程≤5天），
#   并购买境内旅游保险（保额：意外身故≥50万、意外医疗≥5万）。
#   老年人出行需要购买适合高龄人群的保险产品，保障应涵盖意外身故和意外医疗。
#
# 业务流程:
#   1. 用户输入：目的地（桂林）、出发日期（10天后）、人数（1位老年人）、需求（跟团游+保险）
#   2. 系统筛选：显示桂林跟团游产品（行程≤5天，适合老年人），境内旅游保险（保额≥50万/5万）
#   3. 用户选择：确认跟团游产品和保险，填写联系人（张建国）、投保人（张建国）
#   4. 确认支付：核对跟团游订单、保险订单信息和总价格
#   5. 完成订单：生成2个订单（TourGroupBooking + InsuranceOrder），保险关联到TourGroupBooking
#   6. 获取凭证：获取跟团游凭证、保险凭证
#
# 复杂度分析:
#   1. **多订单关联逻辑**（中）：需创建2个不同类型订单（跟团游、保险），保险需关联到TourGroupBooking
#   2. **老年人保险筛选**（高）：需根据老年人特殊需求选择适合的保险产品（意外身故≥50万、意外医疗≥5万）
#   3. **保险日期匹配**（中）：保险起始日期需与跟团游出发日期一致，保险天数需与行程天数一致
#   4. **跟团游产品筛选**（中）：需根据目的地和行程天数（≤5天）筛选适合老年人的跟团游产品
#   5. **联系人/投保人信息验证**（低）：验证联系人和投保人信息完整性
#
# 评分标准（总分100%）:
#   - 创建了跟团游订单 (20%) - 跟团游订单存在性
#   - 预订了1位老年人 (5%) - 人数准确性
#   - 创建了保险订单 (15%) - 保险订单存在性
#   - 保险类型正确（境内旅游保险） (15%) - 保险类型验证（核心要求）
#   - 保险保额适合高龄人群 (15%) - 保额验证（核心要求，高权重）
#   - 保险日期与跟团游匹配 (5%) - 日期一致性
#   - 保险天数与行程天数一致 (5%) - 天数一致性
#   - 联系人信息正确（张建国） (10%) - 联系人信息完整性
#   - 投保人信息正确（张建国） (5%) - 投保人验证
#   - 订单状态有效 (5%) - 订单状态验证
module V251V300
  class V260BookSeniorTravelWithInsuranceValidator < BaseValidator
    ...
    self.title = '帮张建国（老年人）预订桂林跟团游（10天后出发，行程≤5天，1位老年人），并购买境内旅游保险（保险类型：境内旅游保险，保额：意外身故≥50万、意外医疗≥5万，保险日期与跟团游匹配，保险天数与行程天数一致）'
