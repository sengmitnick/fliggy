class TourGroupsController < ApplicationController
  ...
  def get_departure_city_groups(destination)
    ...
    city_regions = {
      ...
      '云南' => { nearby: ['昆明', '大理', '丽江', '香格里拉', '西双版纳', '腾冲'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '贵阳'] },
      '四川' => { nearby: ['成都', '重庆', '绵阳', '九寨沟', '峨眉山', '乐山'], others: ['北京', '上海', '杭州', '广州', '深圳', '西安', '昆明', '兰州'] },
      '贵阳' => { nearby: ['贵阳', '昆明', '成都', '重庆', '南宁'], others: ['北京', '上海', '杭州', '广州', '深圳', '西安', '长沙', '武汉'] },
      
      # 西北地区
      '西安' => { nearby: ['西安', '咸阳', '宝鸡', '渭南', '汉中'], others: ['北京', '上海', '杭州', '成都', '重庆', '郑州', '兰州', '银川'] },
      '青海' => { nearby: ['西宁', '兰州', '西安', '银川'], others: ['北京', '上海', '杭州', '成都', '重庆', '广州', '深圳', '郑州'] },
      '新疆' => { nearby: ['乌鲁木齐', '阿勒泰', '伊宁', '喀什'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '西安', '兰州'] },
      '西藏' => { nearby: ['拉萨', '林芝', '日喀则', '昌都'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '西安'] },
      
      # 华中地区
      '武汉' => { nearby: ['武汉', '长沙', '南昌', '合肥', '郑州'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '西安'] },
      
      # 其他热门旅游目的地
      '海南' => { nearby: ['三亚', '海口', '儋州', '琼海'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '南宁'] },
      '广西' => { nearby: ['桂林', '南宁', '北海', '柳州'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '长沙'] },
      '张家界' => { nearby: ['长沙', '张家界', '武汉', '南昌'], others: ['北京', '上海', '杭州', '广州', '深圳', '成都', '重庆', '西安'] },
      '黄山' => { nearby: ['黄山', '合肥', '杭州', '上海', '南京'], others: ['北京', '苏州', '无锡', '宁波', '福州', '南昌', '武汉', '郑州'] }
    }
    ...
    nearby_title = if destination.to_s.include?('杭州')
      ...
      '杭州及周边参团'
    elsif destination.to_s.include?('云南')
      '昆明及周边参团'
    elsif destination.to_s.include?('四川')
      '成都及周边参团'
    elsif destination.to_s.include?('青海')
      '西宁及周边参团'
    elsif destination.to_s.include?('新疆')
      '乌鲁木齐及周边参团'
    elsif destination.to_s.include?('西藏')
      '拉萨及周边参团'
    elsif destination.to_s.include?('海南')
      '三亚及周边参团'
    elsif destination.to_s.include?('广西')
      '桂林及周边参团'
    elsif destination.to_s.include?('张家界')
      '长沙及周边参团'
    elsif destination.to_s.include?('黄山')
      '黄山及周边参团'
    else
