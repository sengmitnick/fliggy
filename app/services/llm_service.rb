...
class LlmService < ApplicationService
  ...
  # Model name mapping for fixing incorrect model names
  MODEL_NAME_MAPPING = {
  ...
  def initialize(prompt:, system: nil, **options)
    ...
    @system = system
    @options = options
    raw_model = options[:model] || ENV.fetch('LLM_MODEL')
    @model = MODEL_NAME_MAPPING[raw_model] || raw_model
