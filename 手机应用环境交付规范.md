...
### 1. 安装 APK（如果需要）
adb install -r apk/app-release-signed.apk

### 2. 清除应用数据
adb shell pm clear ai.clacky.trip01

### 3. 配置全局设置（API 端点）
adb shell settings put global app_api_endpoint http://192.168.1.10:5010

### 4. 打开 deeplink，

adb shell am start -a android.intent.action.VIEW -d "ai.clacky.trip01://?task_id=02" 
...
供应商可选择交付 Dockfile、或者源码的方式（最好是 docker 的形式，我们内部可以直接一键部署），整个服务 + 存储可独立部署在一台 8核32G 的服务器上，具体要求如下：
  ...
  - **⚠️ 重要：服务器必须绑定在 0.0.0.0（所有网络接口），不能只绑定 localhost/127.0.0.1**
    - Rails 启动命令示例：`bin/dev -b 0.0.0.0 -p 5010` 或 `rails s -b 0.0.0.0 -p 5010`
    - 确保手机可以通过局域网 IP 访问服务器
...
```
{
    ...
    "verification": {
        ...
        "config": {
          "domain": "xiaohongshu",  // verify 的任务类型，如 xiaohongshu 应用的 verify
          "verify_api": "/api/verify/run", // 具体用于 verify 的接口路径，POST，参数为 task_id / session_id
          "params": {
              "task_id":""
              // session_id 是在申请手机实例的运行时产生，算法侧记录 session_id 用于发起对 verfiy_api 的请求 
          }
    ...
    "meta_data": { // 如果供应商对于题目难度和题目类型有增量标签，可以全部归档在meta_data 中
        "difficulty": "easy" // 仅提供一个例子，meta data 中包含的内容供应商可以自行决定。
    } 
}
```

以 v001_book_budget_hotel_validator 为列：

```
{
    "id":"c0342467-8568-4bce-964c-4133c8367e7d",
    "task":{
        "instruction": "今天是2026年02月26日。给张三预订后天入住深圳的酒店（入住1晚，1间房1成人0儿童，预算≤500元/晚，选性价比最高的）"， // 任务描述
        "simulated_user_known_info": "",  // 预留字段
        "simulated_user_persona": "",  // 预留字段
        "success_criteria": "***"  // 预留字段
    },
    "env_id": "v001_book_budget_hotel_validator",
    "version": "1", // 需要和 verify 版本一致，以确保 verify 可在相同版本的环境中执行
    "verification": {
        "driver": "vm_http",  // 供应商不需要关心，先默认填写 vm_http
        "config": {
          "domain": "trip01",  // verify 的任务类型，如 xiaohongshu 应用的 verify
          "verify_api": "/api/verify/run", // 具体用于 verify 的接口路径，POST，参数为 task_id / session_id
          "params": {
